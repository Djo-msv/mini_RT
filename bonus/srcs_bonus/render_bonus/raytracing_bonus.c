...
static t_fcolor	ft_color_ray(t_hit hit, t_scene s, t_ray r, t_fcolor l_int)
{
	...
	while (tmp)
	{
		...
		r.direction = normalize(vec_sub(((t_light *)tmp->content)->coordinate,
					r.origin));
		n_h = intersectscene(s, r, true);
		if (n_h.t <= 0.0 || length(vec_sub((
				(t_light *)tmp->content)->coordinate, r.origin)) < n_h.t)
		{
			i = scalar_product(hit.normal, r.direction);
			if (i > 0.0)
			{
				l_c = scale_mlx_color(mlxcolor_to_fcolor(((t_light *)tmp->content)->color),
						i * ((t_light *)tmp->content)->brightness);
